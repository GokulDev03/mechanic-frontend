...
/* ---------------------------------------------------------------------- */
/* Reusable primitives — same site UI language, teal + amber palette      */
/* ---------------------------------------------------------------------- */
  ...
  return (
    <div className="mb-10 text-center">
      <span
        className={
          tone === "teal"
            ? "inline-block rounded-full bg-teal-100 px-4 py-1 text-sm font-semibold text-teal-700"
            : "inline-block rounded-full bg-amber-100 px-4 py-1 text-sm font-semibold text-amber-600"
        }
      >
        {eyebrow}
      </span>
...
function SecondaryButton({ href, children }: { href: string; children: ReactNode }) {
  return (
    <Link
      href={href}
      className="inline-flex items-center justify-center rounded-lg border-2 border-teal-300 bg-white/10 px-6 py-3 font-semibold text-white shadow-sm backdrop-blur-sm transition-all duration-300 hover:-translate-y-0.5 hover:bg-white/20"
    >
      {children}
    </Link>
  );
  ...
  return (
    <>
      ...
      <main className="bg-white text-gray-900">
        ...
        {/* ---------------------------------------------------------- */}
        {/* Hero — deep teal, amber badge, angled divider into the page */}
        {/* ---------------------------------------------------------- */}
        <header className="relative overflow-hidden bg-gradient-to-br from-teal-950 via-teal-900 to-gray-900">
          <div className="absolute inset-0">
            <Image
              src={service.heroImage}
              alt={`A S SERVICE technician performing ${service.serviceName.toLowerCase()} in ${location.name}`}
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-teal-950/90 via-teal-950/50 to-transparent" />
          ...
          <div className="relative mx-auto max-w-7xl px-4 py-20 sm:px-6 lg:px-8 lg:py-28">
            ...
            <p className="mt-5 max-w-2xl text-lg text-teal-100">
              {t(
                `From repair and installation to gas filling and emergency fixes — A S SERVICE brings certified ${service.serviceNameShort.toLowerCase()} technicians to your doorstep in {location}, the same day you call.`
              )}
            </p>
          ...
          {/* Angled divider — the amber seam between the teal hero and the white page below */}
          <div
            className="absolute bottom-0 left-0 h-3 w-full bg-amber-500"
        ...
        {/* ---------------------------------------------------------- */}
        {/* Why Choose Us — cards with a teal accent stripe             */}
        {/* ---------------------------------------------------------- */}
        <section aria-labelledby="why-choose-us-heading" className="py-16">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <SectionHeading
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
              {service.whyChooseUs.map((item) => (
                <article
                  key={item.title}
                  className="rounded-xl border border-t-4 border-gray-100 border-t-teal-500 bg-white p-6 shadow-lg transition-all duration-300 hover:-translate-y-1 hover:shadow-xl"
                >
                  <h3 className="text-lg font-semibold text-gray-900">{item.title}</h3>
                  <p className="mt-2 text-sm text-gray-600">{t(item.desc)}</p>
                </article>
        ...
        {/* Our Services                                                */}
        {/* ---------------------------------------------------------- */}
        <section aria-labelledby="services-heading" className="bg-teal-50/60 py-16">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <SectionHeading
              eyebrow="Our Services"
              title={`Complete ${service.serviceName} in ${location.name}`}
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <ul className="flex flex-wrap justify-center gap-4">
              {service.brands.map((brand) => (
                <li
                  key={brand}
                  className="rounded-full border border-gray-200 bg-white px-5 py-2 text-sm font-medium text-gray-700 shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:border-teal-400 hover:text-teal-700 hover:shadow-md"
                >
                  {brand}
                </li>
              ))}
        ...
        {/* ---------------------------------------------------------- */}
        {/* Common Problems — left accent bar instead of plain card     */}
        {/* ---------------------------------------------------------- */}
        <section aria-labelledby="problems-heading" className="bg-teal-50/60 py-16">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <SectionHeading
              eyebrow="Common Issues"
              title={`Common ${service.serviceNameShort} Problems We Fix in ${location.name}`}
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {service.problems.map((problem) => (
                <article
                  key={problem.title}
                  className="rounded-xl border-l-4 border-teal-500 bg-white p-6 shadow-lg transition-all duration-300 hover:-translate-y-1 hover:shadow-xl"
                >
                  <h3 className="text-lg font-semibold text-gray-900">{problem.title}</h3>
                  <p className="mt-2 text-sm text-gray-600">{problem.desc}</p>
                </article>
        ...
        {/* ---------------------------------------------------------- */}
        {/* Pricing — teal table header instead of amber                */}
        {/* ---------------------------------------------------------- */}
        <section aria-labelledby="pricing-heading" className="py-16">
          <div className="mx-auto max-w-5xl px-4 sm:px-6 lg:px-8">
            <SectionHeading
            ...
            <div className="overflow-hidden rounded-xl border border-gray-100 bg-white shadow-lg">
              <table className="w-full text-left">
                ...
                <tbody>
                  {service.pricing.map((row, idx) => (
                    <tr key={row.service} className={idx % 2 === 0 ? "bg-white" : "bg-teal-50/40"}>
                      <td className="px-6 py-4 text-sm text-gray-800">{row.service}</td>
                      <td className="px-6 py-4 text-sm font-semibold text-gray-900">
                        {row.price}
                      </td>
        ...
        {/* ---------------------------------------------------------- */}
        {/* Why Local Customers Choose Us — the teal + amber anchor     */}
        {/* ---------------------------------------------------------- */}
        <section aria-labelledby="local-heading" className="bg-gradient-to-br from-teal-950 via-teal-900 to-gray-900 py-16 text-white">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <div className="mb-10 text-center">
              <span className="inline-block rounded-full bg-amber-500 px-4 py-1 text-sm font-semibold text-white">
                Trusted Locally
              ...
              <p className="mx-auto mt-3 max-w-2xl text-base text-teal-100">
                {t(
                  `We're not a call-center dispatch — we're a local ${service.serviceNameShort.toLowerCase()} repair team that knows {location}'s homes, apartments, and shops.`
                )}
              </p>
            ...
            <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
              {[
                ...
                >
                  <h3 className="text-lg font-semibold text-white">{item.title}</h3>
                  <p className="mt-2 text-sm text-teal-100">{item.desc}</p>
                </article>
              ))}
            </div>
          </div>
        ...
        <section aria-labelledby="doorstep-heading" className="py-16">
          <div className="mx-auto grid max-w-7xl gap-10 px-4 sm:px-6 lg:grid-cols-2 lg:px-8">
            ...
            <div className="flex flex-col justify-center">
              <span className="inline-block w-fit rounded-full bg-teal-100 px-4 py-1 text-sm font-semibold text-teal-700">
                Fast & Convenient
              </span>
              <h2 id="doorstep-heading" className="mt-4 text-3xl font-bold text-gray-900 sm:text-4xl">
                Same-Day Doorstep {service.serviceName} in {location.name}
              ...
              <ul className="mt-6 space-y-3">
                {[
                  ...
                  <li key={point} className="flex items-start gap-3">
                    <span className="mt-1 h-2 w-2 flex-shrink-0 rounded-full bg-teal-500" aria-hidden="true" />
                    <span className="text-sm text-gray-700">{point}</span>
                  </li>
                ))}
              </ul>
        ...
        {/* Service Areas                                               */}
        {/* ---------------------------------------------------------- */}
        <section aria-labelledby="areas-heading" className="bg-teal-50/60 py-16">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <SectionHeading
              eyebrow="Service Areas"
              title={`${service.serviceName} Coverage Across ${location.name} & Nearby Areas`}
            ...
            <div className="mt-8 flex flex-wrap justify-center gap-4 text-sm">
              {nearbyLocationLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="rounded-full border border-teal-300 px-4 py-2 font-medium text-teal-700 transition-colors hover:bg-teal-50"
                >
                  {link.label}
                </Link>
              ))}
        ...
        {/* ---------------------------------------------------------- */}
        {/* Customer Reviews — stars stay amber (a rating color, not a brand color) */}
        {/* ---------------------------------------------------------- */}
        <section aria-labelledby="reviews-heading" className="py-16">
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            <SectionHeading
        ...
        {/* FAQ                                                         */}
        {/* ---------------------------------------------------------- */}
        <section aria-labelledby="faq-heading" className="bg-teal-50/60 py-16">
          <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
            <SectionHeading
              eyebrow="FAQ"
              title={`Frequently Asked Questions About ${service.serviceName} in ${location.name}`}
              ...
              {service.faqs.map((faq) => (
                ...
                >
                  <summary className="flex cursor-pointer list-none items-center justify-between text-left text-base font-semibold text-gray-900">
                    {t(faq.q)}
                    <span
                      className="ml-4 flex-shrink-0 text-teal-600 transition-transform duration-300 group-open:rotate-45"
                      aria-hidden="true"
                    >
                      +
                    </span>
        ...
        {/* ---------------------------------------------------------- */}
        {/* Call To Action — stays amber, the site's established booking color */}
        {/* ---------------------------------------------------------- */}
        <section aria-labelledby="cta-heading" className="py-16">
          <div className="mx-auto max-w-5xl px-4 text-center sm:px-6 lg:px-8">
            <div className="rounded-2xl bg-gradient-to-br from-amber-500 to-amber-600 px-6 py-12 shadow-xl sm:px-12">
              <h2 id="cta-heading" className="text-3xl font-bold text-white sm:text-4xl">
                Need {service.serviceName} in {location.name} Today?
              </h2>
              <p className="mx-auto mt-4 max-w-xl text-amber-50">
              ...
              <ul className="flex flex-wrap justify-center gap-4 text-sm">
                {sisterServiceLinks.map((link) => (
                  <li key={link.href}>
                    <Link
                      href={link.href}
                      className="rounded-full border border-gray-200 px-4 py-2 font-medium text-gray-700 transition-colors hover:border-teal-400 hover:text-teal-700"
                    >
                      {link.label}
                    </Link>
                  </li>
